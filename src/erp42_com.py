...
class ERP42_Serial:
    ...
    def __init__(self):
        ...
        #ERP42 프로토콜 값
        self.STX = (0x53, 0x54, 0x58) #프로토콜 지정값 B B B
        self.AorM = 0x01 #auto mode B
        self.ESTOP = 0x00 #B
        self.GEAR = 0 #0,1,2 B
        ...
        self.ETX = (0x0D,0x0A) #프로토콜 지정값 BB
    ...
    def serial_begin(self):
        self.PORT = str(rospy.get_param("~PORT", "/dev/ttyUSB0")) #launch파일에서 고칠 수 있게
    ...
    def set_params(self,GEAR=1, SPEED=0, STEER=0, BRAKE=0):
        self.ALIVE = (self.ALIVE + 1) % 255 #ALIVE #패킷 송신마다 1씩 증가 범위 0~255
    ...
    def Serial_write(self):
        data = self.set_params(self.GEAR, self.SPEED, self.STEER, self.BRAKE) #ERP42 프로토콜 형식의 송신 데이터 생성
                
        data_pack = struct.pack('>BBBBBBHhBBBB', *data) #Big Endian 형식으로 패킷을 Byte 데이터로 변환
    ...
    def Serial_read(self):
        if self.SERIAL.readable():
            data = self.SERIAL.read(18) #18바이트 읽기
            raw_data = Int16MultiArray()
            raw_data.data = list(data) #일단 처리 전 데이터 받기
            #rospy.loginfo(f'data recieved:{list(data)}')
            #rospy.loginfo(type(list(data)))
            data = struct.unpack('<BBBBBBHhBiBBB', data) #Little Endian (PCU to UPPER) 언팩해서 튜플 만들기
            rospy.loginfo(f'data recieved:{data}')

            status_msg = ERP_STATUS() #ERP_STATUS 타입의 ROS 메시지 객체를 생성
            ...
            self.pub1.publish(status_msg) #pub
            self.pub2.publish(raw_data) #pub
              

    def serial_test(self):
        while True:
            if self.SERIAL.readable(): #시리얼 통신 가능하면
                data = self.SERIAL.read(18) #18바이트 읽기
                stx = (data[0],data[1],data[2])
                rospy.loginfo(f'data recieved:{data}')

                if stx != self.STX: #STX 정보가 잘못되어 있으면 다시 열기
    ...
    def close_serial(self):
        if self.SERIAL.is_open: #시리얼이 열려있으면 종료
            self.SERIAL.close()       
        

def main():
    rospy.init_node('erp42_serial') #erp42_serial노드 만들기
    rospy.loginfo("Start ERP-42")
 
    
    erp42_serial = ERP42_Serial() #객체 만들기
